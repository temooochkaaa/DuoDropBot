...
from config import MAX_ACTIVE_NUMBERS, GROUP_ID
...
def process_number(update, context, platform, state):
    ...
    with get_cursor() as cur:
        ...
        if active_count >= MAX_ACTIVE_NUMBERS:
            ...
            update.message.reply_text(
                ...
                f"Ваша позиция в очереди могла бы быть: {queue_pos}",
                reply_markup=back("submit_menu")
            )
            return state
    ...
    context.bot.send_message(
        GROUP_ID,
        f"🆕 Новый номер {platform.upper()}: {phone}\nПозиция: {queue_pos}"
    )
